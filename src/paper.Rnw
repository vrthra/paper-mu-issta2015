...
c.df <- colorize.it(stats.df)

stats.xp <- subset(c.df, grepl('^(r|rr|rc)[.]', operator)) # We are ignoring ceil here. add rc if needed.

o.stats.sel.xp <- subset(o.f(stats.xp), select=sel.colnames)
r.stats.sel.xp <- subset(r.f(stats.xp), select=sel.colnames)
stats.rest <- subset(c.df, !grepl('^(r|rr|rc)[.]', operator))
...
stats.xop <- subset(stats.rest, grepl('^OP', operator))
stats.rest <- subset(stats.rest, !grepl('^OP', operator))

o.stats.sel.xop <- subset(o.f(stats.xop), select=sel.colnames)
r.stats.sel.xop <- subset(r.f(stats.xop), select=sel.colnames)
colnames(o.stats.sel.xop) <- new.colnames
colnames(r.stats.sel.xop) <- new.colnames

o.stats.sel.xop$scope  <- 'operator'
o.stats.sel.xop$sampling  <- gsub('_per_op','',gsub('OP[.]1x','1/', o.stats.sel.xop$operator))
r.stats.sel.xop$scope  <- 'operator'
r.stats.sel.xop$sampling  <- gsub('_per_op','',gsub('OP[.]1x','1/', r.stats.sel.xop$operator))

o.stats.sel.xop$operator  <- gsub('_per_op','',gsub('OP[.]1x','1/', o.stats.sel.xop$operator))
r.stats.sel.xop$operator  <- gsub('_per_op','',gsub('OP[.]1x','1/', r.stats.sel.xop$operator))
