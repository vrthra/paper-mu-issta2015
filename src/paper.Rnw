...
sdl.est <- sdl_s$coefficients['mut.avg.sdl','Estimate']
...
hist(log(10,m$pit.mutation.total.sdl), breaks=50, col=rgb(0,0,1,1/4), main='histogram of log(sdl) mutation score')
hist(log(10,m$pit.mutation.num.sdl), breaks=50, add=T, col=rgb(1,0,0,1/4))

hist(log(10,m$pit.mutation.total.rnd), breaks=50, col=rgb(0,0,1,1/4), main='histogram of log(rnd) mutation score')
hist(log(10,m$pit.mutation.num.rnd), breaks=50, add=T, col=rgb(1,0,0,1/4))
...
$ \mu_\{M_{all} | M_{sdl}\} = 0 + \Sexpr{sdl.est} \times M_{sdl} \text{ with } R^2 = \Sexpr{sdl.r} $

\subsubsection{mut.avg.rnd and mut.avg}

$ \mu_\{M_{all} | M_{rnd}\} = 0 + \Sexpr{rnd.est} \times M_{rnd}  \text{ with } R^2 = \Sexpr{rnd.r} $
...
<<>>=
s <- t.test(mut.avg.rnd, mut.avg.sdl, paired=T)
@
...
<<results='asis'>>=
s <- subset(m, select=c(mut.avg,mut.avg.sdl))
s$type <- 'sdl'
colnames(s) <- c('mut', 'sample', 'type')
r <- subset(m, select=c(mut.avg,mut.avg.rnd))
r$type <- 'rnd'
...
The number of mutators in RND is
 \Sexpr{sum(m$pit.mutation.total.rnd) / sum(m$pit.mutation.total.sdl)} times SDL.

The total mutants for all is \Sexpr{sum(m$pit.mutation.total)}, for rnd is \Sexpr{sum(m$pit.mutation.total.rnd)} and for sdl is \Sexpr{sum(m$pit.mutation.total.sdl)}

TODO: We should check if the distribution significantly different too.
TODO: Use sampling
TODO: Check on randoop samples too
TODO: Implement bomb (athrow) and see its mutation score, and see how close it comes.
