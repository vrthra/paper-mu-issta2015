...
<<results='asis'>>=
o <- subset(stats.df[with(stats.df, order(-mutant.reduction, -r.square)),], mutant.reduction != Inf & kind == 'original')
o.x <- subset(o, select=c('operator', 'r.square', 'mutant.reduction', 'score.multiplier'))
colnames(o.x) <- c('operator', 'r^2', 'm.reduction', 'mcov.mult')
...
@
<<results='asis'>>=
r <- subset(stats.df[with(stats.df, order(-mutant.reduction, -r.square)),], mutant.reduction != Inf & kind == 'randoop')
r.x <- subset(o, select=c('operator','r.square', 'mutant.reduction', 'score.multiplier'))
colnames(r.x) <- c('operator', 'r^2', 'm.reduction', 'mcov.mult')
