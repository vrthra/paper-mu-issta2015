...
icol <- '#9ECAE0'
#icol <- 'lightgray'
with(o.all,boxplot(values~ind,horizontal=T,border=c(bcol), col=icol, pch='.',main='Original'))
o.det <- cbind(projects = rownames(o.pit.det), stack(o.pit.det))
o.det$ind <- clean(o.det$ind)
with(o.det,boxplot(values~ind,horizontal=T,border=c('black'), add=T,pch='.',  yaxt="n"))

r.all <- cbind(projects = rownames(r.pit.all), stack(r.pit.all))
r.all$ind <- clean(r.all$ind)
with(r.all,boxplot(values~ind,horizontal=T,border=c(bcol),col=icol, pch='.',main='Randoop'))
r.det <- cbind(projects = rownames(r.pit.det), stack(r.pit.det))
r.det$ind <- clean(r.det$ind)
with(r.det,boxplot(values~ind,horizontal=T,border=c('black'), add=T,pch='.', yaxt='n'))
@
