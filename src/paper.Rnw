...
<<>>=
c <- cor(subset(m, select=c(mut.avg.sdl, mut.avg)) , method="kendall", use="pairwise")['mut.avg.sdl', 'mut.avg']
d <- cor(subset(m, select=c(mut.avg.rnd, mut.avg)) , method="kendall", use="pairwise")['mut.avg.rnd', 'mut.avg']
@
