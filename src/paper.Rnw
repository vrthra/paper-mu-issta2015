...
<<results='asis'>>=
o <- subset(stats.df[with(stats.df, order(-r.square, -mutant.reduction)),], mutant.reduction != Inf & kind == 'original')
...
@
<<results='asis'>>=
r <- subset(stats.df[with(stats.df, order(-r.square, -mutant.reduction)),], mutant.reduction != Inf & kind == 'randoop')
