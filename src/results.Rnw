...
are given in Table~\ref{tbl:xselsample}. The result of its variation, using
...
program element. Similarly the Table~\ref{tbl:onesel} contains the results for
the criteria of one mutant per program element.

The same (only for original test suite) is visualized in Figure~\ref{fig:xroundsel}.
