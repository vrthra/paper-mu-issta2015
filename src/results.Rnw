...
program element. Similaryl the Table~\ref{tbl:onesel} contains the results for
the criteria of one mutant per program element.

Finally Table~\ref{tbl:opsel} contains the results for operator selectionl. Unlike
the other tables, the first column here contains the kind of operator selection
applied. We have tabulated the results of applying particular kinds of operator
selection. The TSDL row is the result of applying only the higher order mutant
equivalent to \textit{statement deletion}. The rows that have prefixes \textit{op:}
are the result of applying only a single mutation operator on the program. The rows
with \textit{opsel:} contains the results of various operator selection mechansisms.
All the \textit{opsel:N-sel} are the results of N-selective operator selection
strategies~\cite{offutt1993anexperimental}. We used the order of operators
determined from Figure~\ref{fig:op} for this purpose.
