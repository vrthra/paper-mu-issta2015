...
is given in Table~\ref{tbl:xselceil}. The left tables show results from the
original set, while right tables show results from generated set.  The first
...
The fourth column ($\mu_{red}$) contains the mutant reduction factor.
This is the average of total mutants produced divided by the number of
mutants sampled by the criteria. The fifth column ($\sigma_{red}$) contains
the standard deviation of the same. A few of the projects did not return
any value for certain sampling criteria for particular mutation reduction
fractions. The sixth column contains the number of projects that actually
had valid values for the criteria being considered. The seventh column
($\beta_1$) is the average multiplier between the sampled mutation score
and the full mutation score.
