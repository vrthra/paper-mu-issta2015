...
ggplot(subset(o.xp.sample, values < 100), aes(power, values, fill=interaction(type,power), dodge=type)) +
...
ggplot(subset(o.xp.round, values < 100), aes(power, values, fill=interaction(type,power), dodge=type)) +
  stat_boxplot(geom ='errorbar')+
  geom_boxplot() + facet_wrap(~type)+ myopts+ theme(legend.position="none")
@
...
ggplot(subset(o.loc.ext, values < 100), aes(power, values, fill=interaction(type,power), dodge=type)) +
  stat_boxplot(geom ='errorbar')+
  geom_boxplot() + facet_wrap(~type) + myopts+ theme(legend.position="none")
@
...
ggplot(subset(o.loc.int, values < 100), aes(power, values, fill=interaction(type,power), dodge=type)) +
  stat_boxplot(geom ='errorbar')+
  geom_boxplot() + facet_wrap(~type) + myopts+ theme(legend.position="none")
@
...
\begin{figure*}[t]
<<op.all,fig.width=10, fig.height=3, out.width='.95\\linewidth', out.heigth='4in', echo=F, message=F, warn=F,fig.lp="fig:">>=
