...
\begin{figure*}[t]
<<x.per.sel.sample,fig.width=10, fig.height=7, out.width='.95\\linewidth', out.heigth='8in', echo=F, message=F, warn=F,fig.lp="fig:">>=
...
ggplot(subset(o.xp.sample, reduction < 100), aes(factor(power), reduction, fill=type, dodge=type)) +
  ...
  xlab(label=expression(all%/%sample)) +
  ylab('reduction') +
...
@
\caption{The effect of sampling reductions, Original set $R^2 > 0.90$.}
\label{fig:powerplot}
\end{figure*}
...
@
\caption{The mean reduction of mutants vs its standard deviation. Original set, only observations with $R^2 > 0.9$ and $2\times \sigma_{red} < \mu_{red}$ is shown here. }
