...
Suppose $M$ and $M_{strategy}$ denote the original set of mutants and  the reduced set of
mutants, respectively. 
% The strategy chooses $|M_{strategy}|$ mutants on its application. 
Test suite composed of test cases that 
are able to kill an $m$ where  $ m \in M_{strategy}$ is given by
$T_{strategy}$. The total number of \texttt{assert}s in $T_{strategy}$
is given by $asserts(T_{strategy})$. We find a minimal test suite that
is capable of killing all mutants under $M_{strategy}$ by a greedy algorithm
...
for baseline, we build set of randomly selected mutants $M_{random}$,
such that  $|M_{random}| = |M_{strategy}|$. Given any set of tests $T$,
...
We  compute it as the difference between the number
...
and negative test utility denotes that the strategy needs less test cases.
Values close to zero basically denote that the strategy rouphly suggest the 
same number of tests. Note that the comparison here is betweeen the size of
tests and it does not imply any subset relationship between test suites. 
