...
\textit{do faster} approaches include byte code based mutation analysis
systems like PIT, and similar binary patching methods.
...
was sufficient for a very correlation (99\%) with the full mutation score, while sampling even
