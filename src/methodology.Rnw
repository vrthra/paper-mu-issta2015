...
generated test suites from Randoop~\cite{randoop}.  Due to limited resources, we had to
further reduce the programs and test suites to those that could finish
mutation analysis in 10 minutes. From this set of test suites, we
sampled 110 suites each for both original and randoop generated
test suites. This gave us 81 unique projects for both original and
Randoop suites, and remaining projects having both flavors of test suites. {\bf ADG I have no idea what that last sentence means, really.}
...
important a mutation samplings relationship to simple code coverage
...
each program element. {\bf ADG:  Could maybe make this clearer?}
