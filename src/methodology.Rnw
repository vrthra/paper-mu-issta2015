...
\noindent\textbf{Offut et al. \cite{offutt1996anexperimental}.} Mothra supports
three main classes of operators; operand mutators, operator (expression)
mutators and statement mutators. The E-Selective strategy chooses only those
...
For mapping to PIT, we have tried to follow the spirit of choosing only
operator mutators form the mutators available in PIT. The operators we have
used are \Pitin, \Pitm, \Pitcb, \Pitnc, \Pitrc, and \Pitnc.
...
\[M = \alpha m_1 + \beta m_2 ... \omega m_n\]

where $\alpha,\beta .. \omega$ are constants. However we take advantage of the
limited set of operators to run a complete subset model comparison to obtain
...
in are \Pitnvmc, \Pitrc, \Pitdc, \Pitrv, \Pitcc, \Pitemv, \Pitm, \Pitri
...
\noindent\textbf{Offut et al. ~\cite{offutt1993experimental}.} They suggested
