...
we sampled 107 suites each for both original and Randoop generated
...
both the distribution of mutants and the relative detection rates of each
...
For a detailed description of each mutant type, please refer to PIT documentation~\cite{pitmut}.
...
\[sample(x) = floor (x + random(0..1))\]
to correctly sample decimal numbers.
Next, we modified this criteria slightly, and instead of
...
Our aim was to \emph{remove} elements that were too simple. That
...
are sensible in C but not in Java programs. For example, deletion of return statement is tolerated in C, not in Java.
Moreover, there were a few operators that were not supported by the PIT, and could not be implemented easily.
...
Namin et al. used \Pitin, \Pitm, \Piti, \Pitnc. Many of these operators were not applicable in Java, and some not in PIT.
