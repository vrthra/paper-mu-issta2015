...
We used the operators given by Offutt et al. \textit{INVERT\_NEGS, MATH, COND\_BOUNDARY,} \textit{NEGATE\_COND},
\subsubsection{Criteria III}
Those given by Javalanche \textit{NEGATE\_COND, VOID\_METHOD\_CALLS, NON\_VOID\_METHOD\_CALLS, MATH, INLINE\_CONSTS, E\_MEMBER\_VARIABLE}
\subsubsection{Criteria IV}
those given by Javalanche \textit{NEGATE\_COND, VOID\_METHOD\_CALLS, NON\_VOID\_METHOD\_CALLS, MATH, INLINE\_CONSTS, E\_MEMBER\_VARIABLE}
\subsubsection{Criteria V}
By Namin et al.\textit{INVERT\_NEGS, MATH, INCREMENTS, NEGATE\_COND}.
We also  compare N-Selective mutation as suggested by Offutt et al.
...
using \textit{VOID-M-CALLS,NONV-M-CALLS,CONSTR-CALLS,E-MEMBER-VAR,RM-INCR}
for simple statements, and using \textit{RM-COND} for control structures,
...
\textit{RETURN-VALS}, which is similar.  The operators for \textit{while,for},
and \textit{if} statements were modeled using \textit{DELETE-COND},
...
\textit{RM-SWITCH} which replaced the first 100 labels with default label,
...
produced by \textit{MATH} operator is very small. We also note that our
