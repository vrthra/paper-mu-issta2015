...
@
\caption{The relative contribution of mutation operators in terms of fraction of mutants generated and detected. The generated mutants are shown with black outline while detected mutants have blue border and shade.}
\label{fig:op}
\end{figure*}
...
When the number of mutants sampled was not a whole number, we used
the formula by Zhang et a.~\cite{zhang2013ase}, using the
