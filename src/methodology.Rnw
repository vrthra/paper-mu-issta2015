...
are provided in Table~\ref{tbl:pitop}. For a detailed description of each,
please see~\cite{pitmut}
...
Finally, to remove random noice, each criteria described has been run four times, and the results are averaged to produce the final result.
...
We used several different sampling criterias, based on different strategies of sampling, and what happens when the number of mutants selected falls below a threshold.
