...
with(o.all,boxplot(reduction~ind,horizontal=T,border=c(bcol), pch='.', axes=F, ylim=c(0,0.6), col=icol ))
with(o.det,boxplot(reduction~ind,horizontal=T,border=c('black'), pch='.',main='Original', cex.axis=0.8, ylim=c(0,0.6),add=T))
...
with(r.all,boxplot(reduction~ind,horizontal=T,border=c(bcol), pch='.', axes=F,ylim=c(0,0.6),  col=icol))
with(r.det,boxplot(reduction~ind,horizontal=T,border=c('black'), pch='.',main='Randoop', cex.axis=0.8,ylim=c(0,0.6),add=T))
@
...
projects, and automatically generated test-suites using randoop.  Due
...
generated test-suits. This gave us 81 unique projects for both original
...
elements.  We sampled in the increasing order of scope,
--- \textit{line}, \textit{method} and \textit{class} (\textit{project}
scope was analyzed as part of x\% selection).
...
Next, we modified this criteria slightly, and instead of choosing a mutant
with probability of decimal part, applied \textit{round(sample)} to round
it to the nearest whole number. We used this as the number of samples to
be chosen from the population. Our aim was to remove elements that was
too simple. That is, if the number of mutants dropped below a threshold
determined by the mutant reduction ratio, that program element did not
contribute any thing to the final result.

Next, we explored in the other direction, by forcing the program elements
to return at least one element by using the \textit{ceil(sample)}. That is,
...
was highly correlated with mutation coverage for a project. This immediately
...
number of mutants between operator selection and random sampling of mutants.

We next extended it to sample from each method instead, but using the number
of lines in the method as the number of mutants to be sampled. This provided
a test of hypothesis of how important the coverage was. This was extended
to class and project scope also.

For the first variant, we applied the x\% sampling to the result of the first
sampling based on line counts. That is, in the case of methods, we first
selected \textit{linecount} mutants each from each method, and applied the x\%
selection to the result of this selection. For the second variant, we did
the sampling in one go, where we chose \textit{linecount/x} number of mutants from
...
one mutant per program element, in the order \textit{line},\textit{method},
\textit{class}.
...
we believe that our procedure is reasonable. An interesting point is that
by this procedure statement deletion is no longer a simple mutation and qualifies
as a higher order mutation operator. In fact, the procedure we outlined can
be used in general to judge the effectiveness of a higher order operator from
the results of first order mutation operators that makeup the higher order
mutant in question.
