...
operator \pitnvmc contributed about 23\% of the total number of mutants
produced while \pitrv contributed about 15\%. Similarly, the blue shaded
...
in the detected mutants was 34\% while that of \pitrv was 7\%.
...
We used several different sampling criteria. We varied the criterias based on the granularity and
scope of sampling, and strategy chosen when the number of mutants to be sampled falls below one.
