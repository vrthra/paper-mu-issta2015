...
selection, in these studies; a strategy of either program elements
based sample or one combining both program element-based sampling
and operator selection was best.
%{\bf ADG: did it combing random with pelem or pelem with operator???}
