...
with size ranging from $50$ to $100,000$ lines (excluding tests), allowing
us to draw more widely applicable statistical inferences.
...
checking for the absence of some (faulty) behavior. If supposing, tests
were equivalent in the number of behaviors that they test, then the best test
suite would be one that included the largest number of non-redundant test
cases~\footnote{We show later why this assumption is valid (Specifically,
assertion distribution was $\mu=1.7$ and $\sigma=2$ )}. This suggests that
we could evaluate different mutation reduction strategies on their ability
to choose mutants such that they are killed by a larger number of
non-redundant test cases. Further, such a technique is not limited by the
adequacy of test suite under consideration.
