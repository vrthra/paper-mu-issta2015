...
detected mutants. Since this evaluation crtieria does not depend on the
mutation-adequacy of the test suite under consideration, we can run the
evaluation using real world projects which almost always have non-adequate
test suites.
